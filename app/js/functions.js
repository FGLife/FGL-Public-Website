			...
			if( $fullScreenEl.length > 0 ) {
				$fullScreenEl.each( function(){
					...
					if( element.attr('id') == 'slider' ) {
						...
						if( $('#slider.with-header').next('#header:not(.transparent-header)').length > 0 && ( $body.hasClass('device-xxl') || $body.hasClass('device-lg') ) ) {
		...
		dataResponsiveHeights: function(){
			...
			if( $dataHeightMd.length > 0 ) {
				$dataHeightMd.each( function(){
					...
					if( $body.hasClass('device-md') ) {
						if( elementHeight != '' ) { 
							if (pixelRatio > 1) elementHeight *= 0.70;
			...
			if( $accordionEl.length > 0 ){
				$accordionEl.each( function(){
					...
					if( elementState != 'closed' ) {
						...
						element.find('.acctitle:eq('+ Number(accordionActive) +')').removeClass('acctitlec').next().show();
					}
					...
					element.find('.acctitle').click(function(){
						if( $(this).next().is(':hidden') ) {
							...
							$(this).next('.acc_content').slideUp("normal");
							$(this).parent().find('.acctitlec').removeClass('acctitlec');
						}
						return false;
					});
				});
